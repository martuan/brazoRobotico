...
void setup() {
	...
	//accionarServoPorIncremento(1,30,100,1000);
	//accionarServoPorIncremento(1,-30,100,1000);
	//accionarServoPorIncremento(1,30,100,3000);

	//accionarServoOrigenDestino(1,INIT_ANGLE_BASE,50,3000);
	//goToHome();
...
void loop() {
	...
	if(modo != ultimoModo){//si cambió el modo, lo imprimo
		...
		switch(modo){
			...
			case 2:
				Serial.println("Rutina cargada (instrucciones multilínea)");
				break;
...
*/
				...
				if(flagRepetitivo == 0){
					...
					cantidadDeInstrucciones = 0;//resetea para que no acumule
				}else if(flagRepetitivo == 1){
					for(int i = 0; i < cantidadDeInstrucciones; i++){//procesa cada una de las instrucciones cargadas
						Serial.print("Instrucción nº: ");
						Serial.println(i + 1);
						accionarServo(arrayDeInstrucciones[i].numServo, arrayDeInstrucciones[i].angulo, arrayDeInstrucciones[i].velocidad, arrayDeInstrucciones[i].demora);	
...
//Toma como origen la última posición del servo y lo incremeta o decrementa tantos grados
void accionarServoPorIncremento(int nombreServo, int angulo, int velocidad, int demora){
	...
	if(anguloFinal >= 0 && anguloFinal <= 180){//chequea ángulo valido para desplazarse
		...
		if(angulo >= 0){//si el desplazamiento es positivo
			for(i = anguloInicial; i <= anguloFinal; i++){     

                valorRetenido = moverServo(nombreServo, i, velocidad);
				
			}

			Serial.print("	angulo actualizado = ");
			Serial.println(valorRetenido);
			arrayUltimoValor[nombreServo - 1] = valorRetenido;	
...
//Toma como origen la última posición del servo y lo lleva al ángulo final
void accionarServoOrigenDestino(int nombreServo, int angulo, int velocidad, int demora){

	int anguloInicial = 0;
	int anguloFinal = 0;
	int i = 0;
	int valorRetenido = 0;
	char flagRecorridoIncremental = 1;
	//Servo servo = arrayServo[nombreServo - 1];
	int servoPin = arrayServoPin[nombreServo - 1];

	Serial.println("AccionarServoOrigenDestino");
	...
	//Puede ser que tenga que desplazarse desde 125º a 95º por ejemplo
	if(anguloFinal > anguloInicial){
		flagRecorridoIncremental = 1;
	}else{
		flagRecorridoIncremental = 0;
	}
	...
	if(anguloFinal >= 0 && anguloFinal <= 180){//chequea ángulo valido para desplazarse
		...
		if(flagRecorridoIncremental == 1){//si el desplazamiento es de menor a mayor
			for(i = anguloInicial; i <= anguloFinal; i++){
		...
		}else{//si el desplazamiento es de mayor a menor
			for(i = anguloInicial; i >= anguloFinal; i--){
...
void goToHome(void){
	...
	Serial.println("INICIO - GO TO HOME");

	Serial.println("Lectura de ángulos");	

	Serial.print("	servo1Base = ");
	Serial.println(servo1Base.read() + 1);

	Serial.print("	servo2LowerA = ");
	Serial.println(servo2LowerA.read() + 1);

	Serial.print("	servo3LowerB = ");
	Serial.println(servo3LowerB.read() + 1);

	Serial.print("	servo4Medio = ");
	Serial.println(servo4Medio.read() + 1);

	Serial.print("	servo5Superior = ");
	Serial.println(servo5Superior.read() + 1);

	Serial.print("	servo6Pinza = ");
	Serial.println(servo6Pinza.read() + 1);
...
if(flagPrimeraVez == 1){
	...
	//inicializa los angulos de cada servo
	arrayUltimoValor[0] = (servo1Base.read() + 1);
	arrayUltimoValor[1] = (servo2LowerA.read() + 1);
	arrayUltimoValor[2] = (servo3LowerB.read() + 1);
	arrayUltimoValor[3] = (servo4Medio.read() + 1);
	arrayUltimoValor[4] = (servo5Superior.read() + 1);
	arrayUltimoValor[5] = (servo6Pinza.read() + 1);
	...
	Serial.println("Posicionamiento lento");
	//Mover 1 grado para poder leer la posición
	/*
	accionarServo(1, INIT_ANGLE_BASE, 200, 1000);
	accionarServo(2, INIT_ANGLE_LOWERA, 100, 1000);
	accionarServo(3, INIT_ANGLE_LOWERB, 50, 1000);
	accionarServo(4, INIT_ANGLE_MEDIO, 50, 1000);
	accionarServo(5, INIT_ANGLE_SUPERIOR, 50, 1000);
	accionarServo(6, INIT_ANGLE_PINZA, 50, 1000);
*/
	
	accionarServoOrigenDestino(1, INIT_ANGLE_BASE, 50, 500);
	accionarServoOrigenDestino(2, INIT_ANGLE_LOWERA, 50, 500);
	accionarServoOrigenDestino(3, INIT_ANGLE_LOWERB, 50, 500);
	accionarServoOrigenDestino(4, INIT_ANGLE_MEDIO, 50, 500);
	accionarServoOrigenDestino(5, INIT_ANGLE_SUPERIOR, 50, 500);
	accionarServoOrigenDestino(6, INIT_ANGLE_PINZA, 50, 500);
...
int moverServo(int nombreServo, int i, int velocidad){
	...
	Serial.print(" ");
	Serial.print(i);
	return i;//devuelve el último ángulo


}


//Es un parche para tener compatibilidad con la función accionarServo
void accionarServo(int nombreServo, int angulo, int velocidad, int demora){

	accionarServoPorIncremento(nombreServo, angulo, velocidad, demora);

}

void leerRutina(void){

String lectura = {};
char inChar = ' ';

idxInstruccion = 0;//resetea el índice del array de instrucciones para que siempre las almacene desde cero
	/*
	...
	*/
	...
	while(Serial.available() > 0 || SerialBT.available() > 0){
		...
		if(Serial.available() > 0){//Por serie
			
			inChar = Serial.read();

			if(inChar != ';'){//por Serie termina con ;
				if(inChar != ' ' && inChar != '\n'){//mientras no aparezca un caracter no permitido, acumula
					lectura += inChar;
				}
				...
				//if(lectura.startsWith("R")){//si comienza con 'R' es repetitivo
				if(lectura == "R"){//si llega 'R' es repetitivo
					Serial.println("Movimiento repetitivo");
					flagRepetitivo = 1;
					
					//se queda con la orden quitando el caracter especial 'R' o 'H'
					//lectura.remove(0);
					//lectura = lectura.substring(1, lectura.length());
					//Serial.println(lectura);

				//}else if(lectura.startsWith("H")){//si llega 'H' va al home
				}else if(lectura == "H"){//si llega 'H' va al home
					goToHome();
					//se queda con la orden quitando el caracter especial 'R' o 'H'
					//lectura.remove(0);
					//lectura = lectura.substring(1, lectura.length());
					//lectura.clear();
					borrarArrayDeInstrucciones();
					//arrayDeInstrucciones[0].angulo = 0;//anular desplazamiento
					Serial.println(lectura);
					flagRepetitivo = 0;
					break;
				}else if(lectura == "S"){//si llega 'S' es no repetitivo
					flagRepetitivo = 0;
				}
//				}else{// si no tiene caracteres especiales (es una o varias órdenes simples)
					//flagRepetitivo = 0;
//				}


				lectura += '\0';//para finalizar el String
				Serial.printf("lectura = %s, longitud = %i \n", lectura, lectura.length());
				//Serial.println(lectura);
				//Serial.println(lectura.length());
				//convierte String a char array
				char arr[lectura.length() + 1]; 
				strcpy(arr, lectura.c_str());

				if(lectura != "R" && lectura != "S" && lectura != "H"){//suma si no es un caracter especial
		...
		}else if(SerialBT.available() > 0){//por Bluetooth


			inChar = SerialBT.read();

			if(inChar != '\n'){//por Bluetooth termina con \n
...
/*
		...
		//lectura.replace('\n', '');
		//Serial.read();//descarta un caracter
		Serial.println(lectura.length());
		//convierte String a char array
		char arr[lectura.length() + 1]; 
		strcpy(arr, lectura.c_str());
...
void borrarArrayDeInstrucciones(void){

	//arrayDeInstrucciones[0].cmd = 0;
	//arrayDeInstrucciones[0].angulo = 0;
	for(int i = 0; i < 50; i++){
		arrayDeInstrucciones[i] = {};
	}
